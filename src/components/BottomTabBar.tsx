...
const TABS = [
   ...
   */
  { href: "/home", label: "홈", Icon: HomeIcon, owns: ["/events", "/sessions", "/polls", "/notifications", "/quizzes"] },
...
/**
 * 회색 알약을 가운데보다 아래로 내리는 길이(px). 음수면 위로 올라갑니다.
 *
 * 위 여백에 더하고 아래 여백에서 빼므로 알약 높이는 그대로이고 자리만 움직입니다.
 * 한때 1px 내려 두었다가, 실기에서 보고 정가운데(0)로 되돌렸습니다.
 * 위아래 여백이 BAR_PADDING ± 이 값이므로 절댓값을 4보다 크게 두면 안 됩니다.
 */
const PILL_DROP = 0;
...
export default function BottomTabBar() {
  ...
  /*
   * 지금 켜둘 탭(주황 + 속이 꽉 찬 아이콘) — 손을 떼면 가게 될 그 탭입니다.
   *
      ...
      >
        ...
        */}
        ...
        {TABS.map(({ href, label, Icon }, index) => {
          ...
          /*
           * 회색 알약이 덮은 탭은 아이콘과 글씨가 브랜드 주황(brand-500)이 되고 아이콘 속까지
           * 꽉 찹니다. 나머지 탭은 먹색(ink)입니다. 알약을 끌면 색이 알약을 따라 옮겨 다닙니다.
           *
           * 2026-09-14 당근 하단 바 그림을 따라 모든 탭을 먹색으로 했다가, 같은 날 사용자가
           * "선택하면 주황색 되는걸로 다시 되돌려줘"라고 해서 고른 탭만 주황으로 돌아왔습니다.
           * 흰 판·판판한 회색 알약·굵은 선·글씨 크기 등 나머지 그림식 모양은 그대로입니다.
           * (당근식 이전에는 안 고른 탭이 ink-soft였는데, 그건 되돌리지 않았습니다.)
           */
          const itemClassName = covered ? "text-brand-500" : "text-ink";
          /*

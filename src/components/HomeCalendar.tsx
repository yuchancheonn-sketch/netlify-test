...
export default function HomeCalendar({ cohort }: { cohort: string }) {
  ...
  return (
    <section
      onClick={centerSelf}
      // pt-[10.5px] — "2026년 9월" 위 흰 여백 (2026-09-25 사용자 "0.5px 줄여줘" → "좀 줄여줘" → "2px 더", 16 → 15.5 → 12.5 → 10.5px). OX 퀴즈와 함께.
      className="rounded-3xl bg-surface px-4 pt-[10.5px] pb-3 shadow-[var(--shadow-card-flat)]"
    >
      {/* 달 이름과 ‹ › */}
      ...
      */}
      <div className="mt-2 grid grid-cols-7 px-1.5 text-center text-[12px] font-medium">
      ...
      <div className="mt-1 grid grid-cols-7 px-1.5">
        {cells.map((day, index) => {
          ...
          */
          /* 가로줄만 긋습니다 — 칸 사이 세로선은 넣었다가 2026-09-24 사용자 요청으로 도로 뺐습니다. */
          const cellLines = "border-t border-line";
          if (day === null) return <span key={`blank-${index}`} className={cellLines} aria-hidden="true" />;
          ...
          return (
            <button
              ...
              // pt-[3px] — 날짜 동그라미를 1px 더 위로 (2026-09-23 사용자 요청, pt-1(4px)에서).

...
export default function PageHeader({
  ...
  return (
    ...
    >
      ...
      {/*
        제목 줄 바로 아래에 깔리는, 흰색에서 투명으로 옅어지는 띠 (2026-09-14).

        ★ 무엇을 고치는 것인가
          제목 줄이 붙박이라 본문이 그 밑으로 지나가는데, 바탕이 흰색으로 딱
          끊겨 있어 지나가는 것이 **가위로 자른 듯 잘려** 보였습니다. 특히
          원우수첩 검색 알약처럼 1.5px짜리 얇은 테두리는 윗변이 조금만 가려도
          부서진 것처럼 읽힙니다. 이 띠 덕에 잘리는 대신 스르르 사라집니다.

        ★ 왜 header 안에 있나
          top-full로 제목 줄의 아래변에 매답니다. 제목 줄이 붙박이라 이 띠도
          함께 따라다닙니다. 바깥에 두면 같이 안 움직입니다.
          (sticky는 자리잡은 상자라 absolute의 기준이 됩니다.)

        ★ 색을 흰색으로 박지 않고 --color-canvas를 읽습니다.
          박아두면 어두운 화면에서 이 자리만 뿌옇게 밝아집니다.
          하단 탭바 뒤의 띠가 --scrim을 읽는 것과 같은 까닭입니다.

        pointer-events-none — 그림일 뿐이라 손끝이 뚫고 지나가야 합니다.
        없으면 띠에 가린 12px 구간의 카드가 안 눌립니다.
      */}
      <div
        aria-hidden="true"

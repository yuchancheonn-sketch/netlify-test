...
/**
 * 그 기수에 대학생 원우가 있을 수 있는지 — **이제 모든 기수가 그렇습니다** (2026-09-22 사용자 요청
 * "1,2기도 10기처럼 전체, 일반, 대학생으로 나눠줘").
 *
 * 그 전에는 1기·2기만 false였습니다. 공식 원우 명단상 그 두 기수에는 대학생 원우가 없었고
 * (3기부터 일반 원우와 대학생 원우가 함께 들어왔습니다), 그래서 그 수첩에서는 일반/대학생을
 * 가르는 고르개·배지를 숨기고 저장도 늘 일반 원우로 했습니다.
 *
 * ★ 지금은 기수를 가리지 않습니다. 1·2기 수첩에도 세 칸이 서고, "대학생 원우"를 누르면
 *   지금은 아무도 없으니 "대학생 원우가 없어요"가 뜹니다(members/page.tsx의 빈 화면).
 *   앞으로 그 기수에 대학생 원우를 올리면 그대로 잡힙니다.
 *
 * ★ 되돌리려면 이 함수만 고치면 됩니다 — 부르는 곳(원우수첩 고르개·배지, 원우 추가 시트,
 *   프로필 편집) 넷이 모두 이 값 하나를 봅니다.
 *
 * 인자를 그대로 둔 것은 되돌릴 때를 위해서입니다.
 */
export function hasYouthMembers(_cohort: string | null | undefined): boolean {
  return true;
}

/**
 * 원우수첩에서 "원우 추가하기"로 새 이름을 올릴 수 있는 기수인지 —
 * **이제 모든 기수입니다** (2026-09-22 사용자 요청 "모든 기수들이 다 원우 추가할 수 있도록").
 *
 * 그 전에는 지금 기수(10기)만이었습니다. 1기~9기 명단은 공식 원우 명단으로 이미 다 넣었으므로
 * (2026-09-11) 그 수첩에서는 추가 단추를 숨기고 추가 시트에서도 고를 수 없게 했었습니다.
 * 빠진 이름이나 뒤늦게 올릴 원우가 생기면서 그 제약을 풀었습니다.
 *
 * (화면에서 막던 것이지 보안 규칙이 막던 것은 아닙니다 — roster 쓰기는 원래 원우 누구나입니다.)
 *
 * ★ 되돌리려면 이 함수만 고치면 됩니다 — 추가 단추, 추가 시트의 기수 목록, 저장 직전 확인
 *   세 곳이 모두 이 값 하나를 봅니다.
 */
export function canAddMembers(_cohort: string | null | undefined): boolean {
  return true;
}

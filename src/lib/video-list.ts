...
/**
 * 도산아카데미 유튜브 영상 목록 받기 — 서버 전용 (2026-09-22에 app/api/videos/route.ts에서 옮김).
 * 소식 탭(/api/videos)과 카카오 챗봇(/api/kakao/skill)이 아래 getVideoList() 하나를 함께 씁니다.
 *
 * 두 가지 길이 있습니다.
...
/**
 * 영상 목록 하나를 돌려줍니다 — 소식 탭(/api/videos)과 카카오 챗봇(/api/kakao/skill)이 함께 씁니다.
 *   - 한 시간 안에 받아 둔 것이 있으면 유튜브에 다시 묻지 않습니다.
 *   - 새로 못 받으면 전에 받아 둔 목록(메모리 → Firestore)을 돌려주고 fallback: true.
 *   - 그것도 없으면 null.
 * cacheSeconds는 CDN이 이 답을 들고 있을 시간입니다(예비 목록이면 짧게).
 */
export async function getVideoList(): Promise<
  { items: VideoItem[]; complete: boolean; fallback: boolean; cacheSeconds: number } | null
> {
  if (lastGood && Date.now() - lastGood.fetchedAt < CACHE_SECONDS * 1000) {
    return { ...lastGood, fallback: false, cacheSeconds: CACHE_SECONDS };
  }
  ...
  if (fresh) {
    lastGood = fresh;
    await save(fresh);
    return { ...fresh, fallback: false, cacheSeconds: CACHE_SECONDS };
  }

  // 유튜브가 안 될 때 — 전에 받아 둔 목록으로.
  const saved = lastGood ?? (await loadSaved());
  if (saved) {
    lastGood = saved;
    console.warn("[videos] 유튜브에서 못 받아 전에 받아 둔 목록을 내보냅니다.");
    return { ...saved, fallback: true, cacheSeconds: FALLBACK_CACHE_SECONDS };
  }
  return null;
}

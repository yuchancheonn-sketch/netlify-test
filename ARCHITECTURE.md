...
> 세 방법은 처음엔 서로 다른 Firebase 계정(uid)이지만, 카카오·휴대폰 계정은 첫 프로필 설정에서 같은 기수·이름·
> **인증된** 휴대폰 번호의 기존 계정에 이어집니다(`accountLinks/{별칭 uid} → primaryUid`, 서버만 읽고 씀).
> 이어진 뒤에는 그 방법으로 로그인해도 서버가 본계정의 로그인 표를 줍니다 — `lib/account-link-server.ts`.

>
> **로그인 없이 둘러보기 (2026-09-24):** 로그인 안 한 사람도 본편 화면을 봅니다(첫 화면은 홈). 올리기·수정·투표·댓글은
> 누르는 순간 로그인 안내 창(`components/LoginRequired.tsx`), 채팅·알림·내 프로필·운영진·역대 퀴즈·일정 등록/수정은
> 안내 상자(`components/GuestGate.tsx`). 보안 규칙은 공개 데이터(일정·소식·위원회·투표·파일·수업·느낀점)의 **읽기만** 열었고,
> 원우 명단(users·roster)은 닫아 둔 채 서버가 **번호·이메일을 뺀** 명단을 줍니다(`/api/public/directory`).
> 로그인 안 하고 푼 퀴즈는 `/api/quiz/check`가 채점만 하고 기록하지 않습니다("나의 등수 보러가기" → 로그인).
...
| **YouTube** (`@dosanacademy`) | 복습 영상 목록 | 키가 있으면 전편, 없으면 채널 RSS로 최근 15편 | 자료 탭 복습 영상 칸만 비어 있음 |
...
>
> **2026-09-22 사용자 요청으로 복습 영상과 행사 사진을 맞바꿨습니다** — 소식 = 행사 사진·소식,
> 자료 = 복습 영상·파일. 위의 "받아오는 것 / 올리는 것" 경계는 이제 없습니다.
> 두 목록은 `components/AlbumList.tsx`, `components/VideoList.tsx`로 떼어 냈습니다.

| 주소 | 화면 | 하는 일 | 읽는 곳 |
|---|---|---|---|
| `/home` | 홈 | OX 퀴즈 + D-day 카드 + 열린 투표 + 바로가기(투표 만들기·의견 모으기·수업 기록·역대 투표) + 이후 일정 + 오늘의 도산 + 도산아카데미 정보 | `events`, `polls` |
| `/sessions` | 수업 기록 | 1~10주차 한 줄씩, 누르면 그 주 화면. 홈 바로가기로 진입, 밀어서 홈으로 | `sessions` |
| `/polls` | 역대 투표 | 그 기수 + 전체 기수 투표를 열린·닫힌 것 모두 최신순. 닫힌 것은 결과만. 홈 바로가기로 진입, 밀어서 홈으로 | `polls` |
| `/events` | 모임 (목록/캘린더 전환) | 지난 일정은 **화면에서만** 감춤. 카드 안에 안내 글, 올린 사람·운영진에게 수정·삭제. 홈 D-day·이후 일정·새 일정 알림이 모두 여기로 옴 | `events` |
| `/events/[id]` | (모임 상세 — 2026-09-11에 없앰) | 옛 알림 주소가 헛걸음하지 않게 `/events`로 넘기기만 함 | — |
...
| `/library` | 자료 (영상/소식/파일) | 복습 영상 = 유튜브, 소식 = 도산아카데미 공지(`?tab=news`), 파일 = 원우가 올린 문서. 새 영상·새 소식 알림이 여기로 옴 | `/api/videos`, `/api/dosan`, `files` |
...
| 컬렉션 | 문서 id | 담는 것 | 규칙 |
|---|---|---|---|
| `users/{uid}` | 로그인 uid | 이름·회사·직책·휴대폰·원우회 직위·소개·소개영상·**프로필 사진 주소(Cloudinary, 2026-09-11 전엔 data URL)**·role·status | 로그인하면 누구나 읽기/쓰기 |
...
| `polls/{id}/tally/counts` | 고정 | **무기명 투표**의 자리별 표 수(`counts[]`)만 — 누가 무엇을 골랐는지 없음 (2026-09-11) | 읽기 누구나 / 만들기는 투표 연 사람(모두 0) / 고치기는 "새 voters 표시와 함께 한 자리 +1"만 |
| `polls/{id}/voters/{uid}` | 투표한 uid | "이미 넣었다"는 표시(`votedAt`)만, 고른 자리 없음 | **본인만 get**, list 막음, 만들기만(고치기·지우기 없음 → 다시 고르기 없음) |
...
| 대상 | 상태 |
|---|---|
| 수첩·명단·사진·수업 | **활짝 열림** — 로그인만 하면 누구나 읽고 씀 (휴대폰 번호 포함). 원우끼리 서로 채워주는 앱이라 일부러 이렇게 둠 |
...
| `feedState` | **규칙에 안 적음 = 클라이언트 전면 차단.** 새 복습 영상·소식 알림의 "이미 본 글 id"(`videos`, `news` 두 문서). Netlify 예약 함수 `netlify/functions/feed-push.mts`(매시)만 씀. `videoList` 문서는 `/api/videos`가 마지막으로 받은 영상 목록(유튜브가 안 될 때 예비로, 2026-09-22) |
...
**돌고 있는 것**: 로그인, 원우수첩, 일정, 무기명 투표·익명 의견, 수업 기록+느낀점, 채팅(단체+1:1), 행사 사진, 복습 영상, 도산 소식, 보기 설정.

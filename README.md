...
> **규칙을 올리기 전까지는 앱이 열려 있는 상태입니다.** Google 계정은 누구나 만들 수
> 있으므로, 로그인만 확인하는 규칙으로는 인터넷의 아무나가 원우 휴대폰 번호까지
> 통째로 가져갈 수 있습니다. 지금 규칙은 **운영진이 확인해준 원우(status: approved)**
> 만 통과시킵니다. 규칙을 올리는 이 단계가 곧 앱의 자물쇠입니다.
...
**문지기는 운영진입니다.** 초대 코드는 쓰지 않습니다. Google 로그인은 누구나
할 수 있지만, 로그인한 사람은 "운영진 확인을 기다리고 있어요" 화면에 멈춥니다.
운영진 화면 → **확인 대기** 에서 눌러주기 전까지는 원우 명단도, 채팅도,
사진도 한 줄도 내려오지 않습니다. 화면에서만 막는 것이 아니라 Firestore 규칙이
막는 것이라, 앱을 거치지 않고 직접 불러도 마찬가지입니다.

그래서 주소가 밖으로 퍼져도 자료는 새지 않습니다. 다만 낯선 사람이 확인 대기에
쌓일 수 있으니, **아는 이름인지 보고 눌러 주세요.** (검색엔진에는 잡히지 않습니다 — `noindex`)

**잘못 확인해준 사람이 보이면** 운영진 화면 → 권한 관리 → **차단** 을 누르면
...
- [x] Google 로그인 → 운영진 확인 → 온보딩 (초대 코드는 쓰지 않음)

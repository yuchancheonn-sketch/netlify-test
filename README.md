...
- **로그인·가입**: 첫 화면에서 "로그인하기" / "회원가입하기" → 구글 · 카톡 · 휴대폰 번호 중 하나 (2026-09-22).
  어느 방법으로든 처음 들어온 원우는 첫 프로필에서 **(문자로 인증한) 휴대폰 번호가 같은** 기존 계정이
  있으면 이름·기수와 상관없이 그 계정 하나로 합쳐집니다(2026-09-23부터, `accountLinks`,
  lib/account-link-server.ts). 로그인하면 바로 입장
  (아래 [비공개성은 이렇게 지킵니다](#비공개성은-이렇게-지킵니다) 꼭 읽어보세요)
- **홈 캘린더** (2026-09-23): 한 달 보기. 우리 기수 모임(`events`) + 도산아카데미 일정(`academyEvents`).
  - 도산아카데미 일정은 서버가 dosan21.kr 새 글의 "일시·장소"를 읽어 스스로 넣습니다
    (lib/academy-calendar-server.ts). 예약 실행이 없어 원우가 홈을 열 때 한 시간에 한 번만 돕니다.
  - "내 폰 캘린더에 연결" — 원우마다 비밀 구독 주소(`/api/calendar/feed/{열쇠}.ics`)를 아이폰·구글 캘린더가
    구독합니다(lib/calendar-feed-server.ts). 폰은 읽기만 하고, 주소가 https라 **배포본에서만** 제대로 됩니다.

---

## 새 컴퓨터에서 이어서 하기 (2026-09-22)

**GitHub 저장소가 유일한 원본입니다.** 코드·설정·배포 방법·작업 규칙(CLAUDE.md)이 모두 여기 있고,
비밀값은 Firebase Secret Manager에 있습니다. 컴퓨터에만 있는 것은 없어야 합니다.
...
운영진은 푸시를 눌러 **원우 소식 탭 → 카톡으로 보내기**(폰 공유 창) → 카카오톡 → 원우 단톡방을 고릅니다.
...
> **들어오는 문은 열려 있습니다.** Google 계정으로 로그인한 사람은 누구나 들어와
> 모든 자료를 읽고 씁니다(원우 휴대폰 번호 포함). 초대 코드도 승인도 쓰지 않기로
> 한 결과입니다. 다만 **자기 손으로 운영진이 되는 것과, 차단당한 뒤에도 자료를
> 보는 것**은 규칙이 막습니다. 문을 세우고 싶어지면 아래 "초대 코드를 다시
> 켜고 싶다면"을 보세요.
...
새 채팅이 오거나 새 일정이 올라오면 원우들 폰에 알림이 뜹니다.
**값 두 개를 넣어야 동작합니다.** 안 넣으면 설정 화면의 "알림"이
"아직 준비되지 않았어요"로 표시되고, 앱의 나머지는 그대로 돌아갑니다.

**① 웹 푸시 인증서 (공개키 — 브라우저에 노출되는 게 정상입니다)**

1. Firebase 콘솔 → **프로젝트 설정(톱니바퀴) → 클라우드 메시징**
2. **웹 푸시 인증서** 항목에서 **키 쌍 생성**
3. 나온 긴 문자열을 `.env.local` 과 Netlify 환경변수에 넣습니다.

```
NEXT_PUBLIC_FIREBASE_VAPID_KEY=여기에_붙여넣기
```

**② 서비스 계정 (★진짜 비밀값★)**

알림을 실제로 쏘는 일은 서버가 합니다. 그러려면 서버가 Firebase에
"나는 이 프로젝트 주인이다"라고 증명할 열쇠가 필요합니다.

1. Firebase 콘솔 → **프로젝트 설정 → 서비스 계정**
2. **새 비공개 키 생성** → JSON 파일이 내려받아집니다.
3. 그 파일을 열어 내용을 **한 줄로 만들어** 통째로 붙여넣습니다. (따옴표로 감싸지 마세요)

```
FIREBASE_SERVICE_ACCOUNT={"type":"service_account","project_id":"aegiaeta10", ... }
```

4. **Netlify 환경변수에도 같은 값을 넣고 다시 배포**해야 배포본에서 알림이 갑니다.

> **이 JSON은 절대 저장소에 커밋하거나 남에게 보내지 마세요.**
> 이걸 가진 사람은 보안 규칙을 건너뛰고 데이터베이스 전체를 읽고 쓸 수 있습니다.
> 실수로 새어나갔다면 서비스 계정 화면에서 그 키를 **삭제**하고 새로 만드세요.
> `NEXT_PUBLIC_` 이 붙지 않았으므로 이 값은 서버에만 남고 브라우저로는 나가지 않습니다.

**원우들에게 안내할 것**

알림은 **기기마다 따로 켜야 합니다.** 앱 안에서 **설정(톱니바퀴) → 알림 → 켜기**
를 누르고, 브라우저가 묻는 **허용**을 눌러야 그 기기로 알림이 옵니다.
폰에서 켜도 태블릿에서는 다시 켜야 합니다. (브라우저 권한이 기기 단위입니다)

> **아이폰은 홈 화면에 추가해야 알림이 옵니다.**
> 사파리에서 그냥 열어둔 상태로는 iOS가 웹 알림을 주지 않습니다.
> 공유 버튼 → **홈 화면에 추가** 로 설치하고, **그 아이콘으로 연 다음** 켜세요.
> (iOS 16.4 이상 필요) 안드로이드 크롬은 설치하지 않아도 됩니다.

**Cloud Functions를 안 쓴 이유**

보통은 "메시지가 저장되면 자동으로 알림"을 Cloud Functions로 만드는데,
그건 유료(Blaze) 요금제를 요구합니다. 그래서 이 앱은 메시지를 보낸 쪽이
`/api/push/chat` 을 직접 두드리고, Netlify에서 도는 서버가 알림을 쏩니다.
Netlify 무료 요금제의 함수 호출 한도는 **월 12만 5천 번**이라 50명 규모에서는
남습니다. (`src/app/api/push/` 와 `src/lib/push-server.ts`)
...
> 다만 권한(운영진 여부)과 승인 상태만은 운영진만 바꿀 수 있습니다.
...
| `npm run dev:reset` | 개발 서버가 느려졌을 때 캐시(`.next/dev`)를 비웁니다 |
| `npm run preview:phone` | 배포본과 같은 빌드를 폰에서 보기 (느릴 때 이걸로) |
...
> 카카오 로그인을 이 주소에서 해 보려면 카카오 개발자 콘솔에 사이트 도메인 `http://172.30.1.8:3000`과
> Redirect URI `http://172.30.1.8:3000/auth/kakao`를 더 등록해야 합니다. 안 하면 카카오 화면에
> **KOE006(앱 관리자 설정 오류)** 이 뜹니다(2026-09-23). IP가 바뀌면 새 IP로 다시 등록합니다.

### 폰에서 너무 느릴 때

개발 서버는 원래 느립니다. 코드를 고치는 즉시 화면에 반영하려고 압축도 최소화도
하지 않은 채로 내려보내기 때문에, 화면 하나에 자바스크립트가 **1.4MB쯤**(압축 후)
따라옵니다. 배포본은 여기서 개발 전용 뭉치가 빠지고 최소화까지 되어 훨씬 가볍습니다.

**1. 배포본과 같은 빌드로 보기** — 고치는 중이 아니라 "다 된 화면"을 볼 때 씁니다.

```bash
npm run preview:phone
```

빌드에 1~2분 걸리고 그동안 자동 새로고침은 안 되지만, 폰에서 체감 속도가
배포본과 같아집니다. Netlify 빌드 시간은 쓰지 않습니다(내 컴퓨터에서 빌드).

**2. 갑자기 느려졌을 때** — 개발 서버가 몇 시간 돌면 `.next/dev` 캐시가 수백 MB로
불어나고, 렌더링 워커가 죽었다 살아나기를 되풀이하면서 CPU를 물고 늘어집니다.
그러면 화면 하나 받는 데 20초씩 걸립니다. 서버를 끄고 캐시를 비운 뒤 다시 켜세요.

```bash
# 개발 서버를 Ctrl+C 로 끈 다음
npm run dev:reset
npm run dev:phone
```

느려졌는지 확인하는 법: 아무것도 안 하고 있는데 작업 관리자에서 `node`가
CPU를 계속 쓰고 있으면 그 상태입니다. 서버 창에
`Jest worker encountered ... exceeding retry limit` 이 반복되면 확실합니다.
...
src/
  app/
    ...
    api/push/chat/          새 채팅 알림을 쏘는 창구
    api/push/event/         새 일정 알림을 쏘는 창구
    firebase-messaging-sw.js/  푸시 알림용 서비스워커 (설정값을 넣어 만들어 내려줍니다)
  ...
  lib/
    ...
    firebase-admin.ts       서버 전용. 알림을 쏠 때만 씁니다 (보안 규칙을 건너뜁니다)
    push.ts / use-push.ts   이 기기에서 알림 켜고 끄기
    push-server.ts          받는 사람들의 기기 토큰을 모아 FCM으로 발송
...
- [x] 내 프로필 (사진·이름·생일·구분·회사·직책·휴대폰·원우회 직위·자기소개) — 별칭은 2026-09-15에 없앰
- [x] 원우수첩 (가나다순 번호 / 이름·회사·직책 검색 / 사진 크게 보기 / 소개 영상 / 원우끼리 서로 정보 수정하기 · 이름 올리기)
- [x] 모임 일정 (목록·월간 캘린더·상세, 원우 누구나 등록 / 수정·삭제는 올린 사람과 운영진)
- [x] 투표·의견 모으기 (투표는 무기명 — 누가 무엇을 골랐는지 어디에도 안 남음, 한 번 넣으면 못 바꿈 / 의견은 익명)
- [x] 단체 채팅 (실시간, 이전 메시지 더 보기, 본명 표시, 안 읽은 개수 배지)
- [x] 보던 탭을 다시 누르면 맨 위로 (채팅 제외)
...
- [x] 웹 푸시 알림 (새 채팅·새 일정 → 폰 알림. 위 [8번](#8-알림-켜기-푸시-알림) 설정 필요)

...
Google 로그인은 전 세계 누구나 할 수 있으므로 초대 코드로 막습니다.

**초대 코드**가 이 앱의 문지기입니다. 코드가 맞아야만 계정 문서가 만들어지고,
코드 검증은 화면이 아니라 **Firestore 보안 규칙**이 합니다.
그래서 앱은 초대 코드 목록을 읽을 권한이 아예 없고, 앱 코드를 고쳐서
검사를 건너뛰려 해도 규칙에서 거절됩니다.

코드가 맞으면 **기다림 없이 바로 입장**합니다. (운영진 승인 단계는 두지 않았습니다.
수십 명 규모에서 승인을 기다리게 하는 것이 번거로워 뺐습니다.)

**잘못 들어온 사람이 있다면** 운영진 화면 → 권한 관리 → **차단** 을 누르면
그 즉시 모든 데이터가 보이지 않게 됩니다.
코드가 유출된 것 같으면 Firestore에서 그 코드의 `active` 를 `false` 로 바꾸고
새 코드를 만들면 됩니다. (이미 들어온 사람은 그대로 남습니다)
...
- 대표 주황: `#FF7210` (`brand-500`) — 테마색·아이콘·그라데이션·배지·강조
- 글자가 올라가는 면: `#C9490A` (`brand-700`) — 버튼·말풍선
- 배경: `#FFFAF6` — 아주 옅은 주황빛 오프화이트

**왜 버튼만 색이 다른가**: `#FF7210` 위에 흰 글씨를 얹으면 명도 대비가 2.7:1로,
접근성 최소 기준(4.5:1)에 크게 못 미쳐 밝은 곳에서 글씨가 잘 보이지 않습니다.
`#C9490A`는 4.7:1로 기준을 통과하면서도 충분히 선명한 주황입니다.
그래서 **글자가 올라가지 않는 면에는 `#FF7210`을, 흰 글씨가 올라가는 면에는
`#C9490A`를** 씁니다.
...
- [x] Google 로그인 → 초대 코드 → 온보딩 (승인 대기 없이 바로 입장)

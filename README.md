...
- 대표 주황: `#FF7210` (`brand-500`) — 버튼·말풍선·테마색·배지
- 홈 화면 아이콘 바탕: 평균 `#EA6209` — 위아래로 살짝 벌린 주황 유리판입니다.
  화면 안에서 알맞은 밝은 주황이 홈 화면에서는 붕 떠 보여 한 단계 진하게 씁니다.
  값은 `scripts/generate-icons.mjs` 한 곳에 있고, 고친 뒤 `npm run icons`.
